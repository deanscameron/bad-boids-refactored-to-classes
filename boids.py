...
class Boids(object):
    ...
    def fly_towards_middle(self):
        ...
        for boid_1 in self.boids:
            for boid_2 in self.boids:
                boid_1.x_vel += update_vel(boid_1.x_pos, boid_2.x_pos, 
												(self.vel_fact/self.num))
                boid_1.y_vel += update_vel(boid_1.y_pos, boid_2.y_pos, 
												(self.vel_fact/self.num))
    ...
    def avoid_nearby_boids(self):   
        ...
        for boid_1 in self.boids:
            for boid_2 in self.boids:
                if dist_check(boid_1.x_pos, boid_2.x_pos, boid_1.y_pos, 
												boid_2.y_pos, self.near_dist):
    ...
    def match_velocities(self):
        ...
        for boid_1 in self.boids:
            for boid_2 in self.boids:
                if dist_check(boid_1.x_pos, boid_2.x_pos, boid_1.y_pos, 
											boid_2.y_pos, self.match_dist):
                    boid_1.x_vel += update_vel(boid_1.x_vel, boid_2.x_vel, 
											(self.match_fact/self.num))
                    boid_1.y_vel += update_vel(boid_1.y_vel, boid_2.y_vel,
											(self.match_fact/self.num))  
